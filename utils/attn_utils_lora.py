...
# ytedance Inc.
# *************************************************************************
# This file have been modified by DragLoRA Authors to add SA_KVs(k,v from pre_saved list) in editor
...
class MutualSelfAttentionControl(AttentionBase):
    ...
    def forward(self, q, k, v, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        if self.guidance_scale > 1.0:
            ...
            batch_size, heads, _, head_dim = q.shape
            k = SA_KV[0].view(batch_size, -1, heads, head_dim).transpose(1, 2)
            v = SA_KV[1].view(batch_size, -1, heads, head_dim).transpose(1, 2)
            out = F.scaled_dot_product_attention(q.to(k.dtype), k, v, attn_mask=None, dropout_p=0.0, is_causal=False)
            out = rearrange(out, 'b h n d -> b n (h d)')

        return out


class MutualSelfAttentionControl_tmp(AttentionBase):
...
def register_attention_editor_unet(unet, editor: AttentionBase, attn_processor='attn_proc'):
    """
    Register a attention editor to Diffuser Pipeline, refer from [Prompt-to-Prompt]
    """
    def register_editor(net, count, place_in_unet):
...
def override_back_attn_proc_forward(attn):
    def forward(x, encoder_hidden_states=None, attention_mask=None, context=None, mask=None):
        """
        The attention is similar to the original implementation of LDM CrossAttention class
        except adding some modifications on the attention
...
# forward function for lora attention processor
# modified from __call__ function of LoRAAttnProcessor2_0 in diffusers v0.17.1
def override_back_lora_attn_proc_forward(attn):
    def forward(hidden_states, encoder_hidden_states=None, attention_mask=None):
        residual = hidden_states
        input_ndim = hidden_states.ndim
        ...
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, channel)
...
def register_back_attention_editor_diffusers(model, attn_processor='attn_proc'):
    """
    Register a attention editor to Diffuser Pipeline, refer from [Prompt-to-Prompt]
    """
    def register_editor(net, count, place_in_unet):
        for name, subnet in net.named_children():
            if net.__class__.__name__ == 'Attention': 
                if attn_processor == 'attn_proc':
                    net.forward = override_back_attn_proc_forward(net)
                elif attn_processor == 'lora_attn_proc':
                    net.forward = override_back_lora_attn_proc_forward(net)
                else:
                    raise NotImplementedError("not implemented")
                return count + 1
            elif hasattr(net, 'children'):
